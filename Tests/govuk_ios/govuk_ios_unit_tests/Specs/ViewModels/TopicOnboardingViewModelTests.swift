...
struct TopicOnboardingViewModelTests {
    ...
    @Test
    func saveTopicsToFavourite_favouritesAllTopicsInSelectedItems() {
        let coreData = CoreDataRepository.arrangeAndLoad
        let topic = Topic.arrange(context: coreData.backgroundContext)
        ...
        sut.selectTopic(topic: topic)
        sut.selectTopic(topic: topicTwo)
        sut.selectTopic(topic: topicThree)

        sut.saveTopicsToFavourite()

        for (_, topic) in sut.selectedTopics {
